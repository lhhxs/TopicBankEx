...
public class ExamDBHelper extends SQLiteOpenHelper{
    ...
    private static final String DB_PATH = "/data/data/com.lurencun.android.topicbank/databases/";
	//如果你想把数据库存放在SD卡的话
	//private static String DB_SD_PATH = Environment.getExternalStorageDirectory().getAbsolutePath()
	//		+ "/mobilesafe/database/";
	
	private static String DB_NAME = (DB_VERSION & 1) == 0? "a.db":"b.db";
...
//			mContext.deleteDatabase(DB_PATH + DB_NAME);
			Log.e("TAG","数据库已存在，do nothing");
		}else{
			Log.e("TAG","新建文件到"+DB_PATH);
			File dir = new File(DB_PATH);
			if(!dir.exists()){
				dir.mkdirs();
			}
			File dbFile = new File(DB_PATH + DB_NAME);
            File bdfile = new File(DB_PATH + ((DB_VERSION & 1) == 0? "b.db":"a.db"));
